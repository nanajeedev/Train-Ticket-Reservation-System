pipeline{
    ...
    stages{
        ...
        stage('build docker image'){
            steps{
                sh 'docker build -t traintickets:v1.1.4 .'
            }
        }
        stage('login docker'){
            steps{       
        ...
        stage('tag image'){
            steps{       
            sh 'docker tag traintickets:v1.1.4 399747338321.dkr.ecr.ap-south-1.amazonaws.com/traintickets:v1.1.4'
            }  
        }
        stage('Push image to registry'){
            steps{       
            sh 'docker push 399747338321.dkr.ecr.ap-south-1.amazonaws.com/traintickets:v1.1.4'
            }  
        }
        ...
        stage('Image,run as container'){
            steps{       
            sh 'docker run -itd --name ${imagename} -p 8084:8080 399747338321.dkr.ecr.ap-south-1.amazonaws.com/traintickets:v1.1.4'
            }  
        }
    }
}